odoo.define('ks_dashboard_ninja_list.ks_image_basic_widget', function (require) {
    ...
    var basic_fields = require('web.form_widgets');
    var QWeb = core.qweb;

    // NOTE
    // add FieldBinaryImage in return statement in form_common module
    ...
    var KsImageWidget = basic_fields.FieldBinaryImage.extend({
        ...
        init: function(field_manager, node) {
            ...
            // console.log( "KsImageWidget" );
        },
        render_value: function () {
            var ks_self = this;
            var url = this.placeholder;
            if (ks_self.get_value()) {
                ks_self.$('> img').remove();
                ks_self.$('> span').remove();
                $('<span>').addClass('fa fa-' +ks_self.view.fields["ks_default_icon"].get_value() + ' fa-5x').appendTo(ks_self.$el).css('color', 'black');
            } else {
